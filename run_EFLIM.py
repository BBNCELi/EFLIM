...
from model import SUPPORT, EFLIM
...
def main():
    ...
    ### Parameters ###
    #######################################################################################
    parser = argparse.ArgumentParser(description="Running EFLIM.")
    ...
    if args.savepath is None:
        args.savepath =  os.path.dirname(args.folderName) + f'//EFLIM'
    ...
    ### Lifetime denoising ###
    #######################################################################################
    model_lifetime = EFLIM(in_channels=args.input_frames, mid_channels=args.unet_channels, depth=args.depth,\
    ...
    results_lt_EFLIM_RGB = inwlt(lifetime_guess[0], intensity_guess[0], lt_contrast, in_contrast, cm, flag_white)
    dropFrames = int((args.patch_size[0] - 1) / 2)
    if dropFrames > 0:
        results_lt_EFLIM_RGB = results_lt_EFLIM_RGB[dropFrames:-dropFrames,:,:,:]

    lt_low, lt_high = lt_contrast
    in_low, in_high = in_contrast
    savename = args.savepath + f"//output_EFLIM_lt{lt_low}-{lt_high}_in{in_low}-{in_high}_{colormapName}.tif"
    saveastiff_RGB(results_lt_EFLIM_RGB, savename)
