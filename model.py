...
from infolution import infolution2d, infolutionHole2d

class SUPPORT(nn.Module):
    """
    The network of SUPPORT for intensity denoising
    https://github.com/NICALab/SUPPORT/blob/main/model/SUPPORT.py
...
class EFLIM(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels=[16, 32, 64, 128, 256], depth=5,\
        ...
        for idx, c in enumerate(last_layer_channels):
            if idx == 0:
                if bp is False and self.twod is False:
                    last_layers.append(nn.Conv2d(2*one_by_one_channels[-1], c, kernel_size=1, padding=0, bias=False))
                else:
                    last_layers.append(nn.Conv2d(one_by_one_channels[-1], c, kernel_size=1, padding=0, bias=False))
            else:
                last_layers.append(nn.Conv2d(last_layer_channels[idx-1], c, kernel_size=1, padding=0, bias=False))
        last_layers.append(nn.Conv2d(c, self.out_channels, kernel_size=1, padding=0, bias=False))
    ...
    def _gen_unet(self):
        ...
        for i in range(len(self.mid_channels)):
            if i == 0:
                self.enc_layers.append(infolution2d(self.in_channels-1, self.mid_channels[i], kernel_size=3, padding=1, bias=False))
            else:
                self.enc_layers.append(nn.Conv2d(self.mid_channels[i-1], self.mid_channels[i], kernel_size=3, padding=1, bias=False))
        self.enc_layers = nn.ModuleList(self.enc_layers)

        # (Unet) decoding layers
        self.dec_layers = []
        for i in range(len(self.mid_channels)-1):
            self.dec_layers.append(nn.Conv2d(self.mid_channels[i] + self.mid_channels[i+1], self.mid_channels[i], kernel_size=3, padding=1, bias=False))
        self.dec_layers = nn.ModuleList(reversed(self.dec_layers))
        ...
        for idx, c in enumerate(self.one_by_one_channels):
            if idx == 0:
                self.unet_1_convs.append(nn.Conv2d(self.mid_channels[0], c, kernel_size=1, padding=0, bias=False))
            else:
                self.unet_1_convs.append(nn.Conv2d(self.one_by_one_channels[idx-1], c, kernel_size=1, padding=0, bias=False))
        self.unet_1_convs = nn.ModuleList(self.unet_1_convs)
    ...
    def forward_bsnet(self, x, unet_out):
        ...
        x_energy = mean_nz(x[0], axis=1).unsqueeze(dim=1)
        for c in range(self.depth3x3):
            if c == 0:
                x1 = x
            else:
                # x1 = x1 + x1.max() * inp
                # print(x.size())
                x1 = x1 + (self.scalars_3x3[c - 1] * x_energy.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        ...
        for c in range(self.depth5x5):
            if c == 0:
                ...
                x2 = x2 + (self.scalars_5x5[c - 1] * x_energy.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
